<%--
    structures.jsp: A custom display style for a structures
    facet. Based on the model of asset_tags.jsp.
        
    Created:    2016-02-22 22:42 by Christian Berndt
    Modified:   2015-02-22 22:42 by Christian Berndt
    Version:    1.0.0
--%>
...
<div class="structures <%= cssClass %>" data-facetFieldName="<%= HtmlUtil.escapeAttribute(facet.getFieldId()) %>" id="<%= randomNamespace %>facet">
    ...
    <ul class="<%= (showAssetCount && displayStyle.equals("cloud")) ? "tag-cloud" : "tag-list" %> nav nav-pills nav-stacked">
        ...
        for (int i = 0, j = 0; i < termCollectors.size(); i++, j++) {
            ...
            String ddmStructureKey = termCollector.getTerm();
            
            DDMStructure ddmStructure = DDMStructureLocalServiceUtil.fetchStructure(scopeGroupId, PortalUtil.getClassNameId(JournalArticle.class), ddmStructureKey);

            String name = ddmStructure.getName(locale);            
        %>
