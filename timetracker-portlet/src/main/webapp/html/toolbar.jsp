...
<aui:nav-bar>
	...
	<aui:nav id="toolbarContainer" cssClass="nav-display-style-buttons">
		...
		<c:if
			test='<%=TimetrackerPortletPermission.contains(
			    ...
			    String taglibAddURL =
			                    "javascript:Liferay.Util.openWindow({id: '" +
			                        ...
			                        HtmlUtil.escapeJS(LanguageUtil.format(
			                            pageContext, "edit-x", "new")) +
			                        "', uri:'" + HtmlUtil.escapeJS(addURL) + "'});";
			%>
		...
		<aui:nav-item dropdown="<%=true%>" id="exportButtonContainer"
			...
			<aui:nav-item>
				<liferay-ui:icon url='<%= HttpUtil.addParameter(exportURL, ns + "format", "csv") %>'
					iconCssClass="icon-download-alt" label="true" message="export-csv" />
			</aui:nav-item>
			...
			<aui:nav-item>
	            <liferay-ui:icon url='<%= HttpUtil.addParameter(exportURL, ns + "format", "xml") %>'
					iconCssClass="icon-download-alt" label="true" message="export-xml" />
			</aui:nav-item>
		</aui:nav-item>
	</aui:nav>
	...
	<aui:nav-bar-search cssClass="pull-right">
		...
		<aui:form action="<%=searchURL%>" method="get" name="fm1">
			...
			<liferay-ui:search-toggle buttonLabel="search"
				displayTerms="<%=displayTerms%>" width="500"
				...
				<aui:fieldset>
					...
					<aui:input dateTogglerCheckboxLabel="ignore-start-date"
						disabled="<%=ignoreStartDate%>" formName="fm" name="startDate"
